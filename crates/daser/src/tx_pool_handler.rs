...
use crate::{Arc, DasKv, Sampling, SamplingClient, SAMPLES_PER_BLOB};
...
use melo_erasure_coding::{
	extend_col::extend_segments_col, recovery::recovery_order_row_from_segments,
};
...
#[derive(Clone)]
pub struct TPListenerParams<Client, H, TP, DB> {
	pub client: Arc<Client>,
	pub das_client: Arc<SamplingClient<H, DB>>,
...
pub async fn start_tx_pool_listener<Client, TP, B, DB, H>(
            ...
            Some(notification) = new_best_block_stream.next() => {
                ...
                let reconstructed_rows: HashMap<usize, Vec<Segment>> = need_reconstruct
                    ...
                    .map(|&row_index| {
                        ...
                        let recovered_row = match recovery_order_row_from_segments(&row, &das_client.network.kzg) {
                ...
                for x in 0..SAMPLES_PER_BLOB {
                    ...
                    match col_result {
                        Some(col) => {
                            match extend_segments_col(&das_client.network.kzg.get_fs(), &col) {
                                Ok(col_ext) => {
