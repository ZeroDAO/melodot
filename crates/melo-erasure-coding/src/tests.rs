...
fn blob_proof_case(field_elements_per_blob: usize, minimize: usize) {
	...
	let (commitment, proof) = blob.commit_and_proof(&kzg, field_elements_per_blob).unwrap();
	// Verify the proof
	let result = blob.verify(&kzg, &commitment, &proof, field_elements_per_blob).unwrap();
	...
	// Verification fails
	let verify = blob.verify(&kzg, &commitment, &proof_mut, field_elements_per_blob).unwrap();
	assert!(!verify);
	// Modify a value in the commit, verification fails
	let commitment_mut = KZGCommitment(commitment.0.add(&FsG1::rand()));
	let verify = blob.verify(&kzg, &commitment_mut, &proof, field_elements_per_blob).unwrap();
	assert!(!verify);
	// Modify the blob
	...
	let verify = blob.verify(&kzg, &commitment, &proof, field_elements_per_blob).unwrap();
	assert!(!verify);
}
...
fn test_blob_verify_batch() {
	...
	let result =
		Blob::verify_batch(&blobs, &commitments, &proofs, &kzg, field_elements_per_blob).unwrap();
	assert!(result);
}
...
fn test_order_segments_col() {
	...
	// Verify if it is correct
	for i in 0..k * 2 {
		// segments_option: if it's Some, then compare the content; if it's None, then directly
		// verify
...
fn test_recovery_row_from_segments() {
	...
	// Get a random Vec of length num_shards, where each number is unique and less than 2 *
	// num_shards
...
fn extend_returns_err_case(num_shards: usize) {
	let kzg = KZG::default_embedded();
...
fn test_bytes_to_segments_round_trip() {
	...
	let segment_datas =
		segments.iter().map(|segment| Some(segment.content.clone())).collect::<Vec<_>>();
...
fn test_bytes_to_segments_case(bytes_len: usize) {
	...
	let blobs = bytes_vec_to_blobs(&vec![bytes.clone()], field_elements_per_blob).unwrap();
	let commitments = blobs.iter().map(|blob| blob.commit(&kzg).unwrap()).collect::<Vec<_>>();
	...
	let segment_count = blobs.len() * 2 * field_elements_per_blob / field_elements_per_segment;
