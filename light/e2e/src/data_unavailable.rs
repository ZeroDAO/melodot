...
pub(crate) async fn run(client: &Client, ws_client: &WsClient) -> Result<()> {
	...
	client.api.rpc().request("das_removeRecords", params.clone()).await?;
	request(ws_client, "das_removeRecords", Some(params)).await?;
	...
	if let Some(confidence) = val {
		if confidence < 999_900 {
			...
			info!(
				"{}: Block confidence is greater than 99.99%: {:?}, it should be unavailable.",
				ERROR, confidence
			);
		}
	} else {
